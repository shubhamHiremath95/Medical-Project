...
class Pos extends Component {
  ...
  generateOrder() {
    if (this.validation()) {
      fetch("https://localhost:7245/api/Sales/SaveOrder", {
        method: "post",
        headers: {
        ...
        crossorigin: true,
        body: JSON.stringify({
          bill_date: new Date().toLocaleString(),
          paid_by: this.state.selectedOption,
          bill_amount:this.state.bill_amount.toString(),
  ...
  onSubmit(event) {
    if (this.validation()) {
      ...
      debugger;
      const tempData = {
        product_id: this.state.productTempData.product_Id.toString(),
      ...
      this.setState({
        ...
        availableQnt: "0",
        selectedProd: "",
        bill_amount: orderData.reduce(function (prev, current) {
  ...
  validation() {
    if (parseInt(this.state.availableQnt) < parseInt(this.state.quantity)) {
  ...
  onProductChange(e) {
    ...
    axios
      .get("https://localhost:7245/api/Sales/GetProductById/" + e.target.value)
      .then((response) => {
        this.setState({
          productTempData: response.data,
          quantity: "",
          availableQnt: response.data.quantity,
        });
      })
      .catch(function (error) {
        console.log(error);
                  ...
                  <Card className="p-2">
                    <CardBody>
                      <Form className="form" onSubmit={this.onSubmit}>
                        ...
                        <div className="row mb-3">
                          <label className="col-sm-4">Available Quantity</label>
                          <div className="col-sm-8">
                            <label> {this.state.availableQnt} </label>
                          </div>
                        </div>
                        <div className="row mb-3">
                          <label className="col-sm-4">Sell Quantity </label>
                          <div className="col-sm-8">
                            <Input
                              name="quantity"
                              type="text"
                        ...
                        <div className="row mb-3">
                          <label className="col-sm-4">Total Amount</label>
                          <div className="col-sm-8">
                            <label> {this.state.bill_amount} </label>
                          </div>
                        </div>
                        <div className="row">
                          <Col sm={3}>Paid By</Col>
                          <div
                            onChange={this.onValueChange.bind(this)}
                            className="col-sm-4"
                          >
                            <label className="col-sm-4">
                              <input type="radio" value="Cash" name="gender" />
                              Cash
                            </label>
                            <label className="col-sm-4">
                              <input
